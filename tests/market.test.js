...
describe("complete-set money layer", () => {
  ...
  it("mint takes sBTC and issues both shares", () => {
    ...
    const r = simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(1_000_000)], alice);
  ...
  it("INVARIANT: vault == idle-circ == bonded-circ across mint and merge", () => {
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(3_000_000)], alice);
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(500_000)], bob);
    simnet.callPublicFn(C, "merge-complete-set", [Cl.buffer(ID), Cl.uint(1_200_000)], alice);
    const m = market();
  ...
  it("merge returns the sBTC 1:1", () => {
    const before = sbtcBalance(alice);
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(2_000_000)], alice);
    simnet.callPublicFn(C, "merge-complete-set", [Cl.buffer(ID), Cl.uint(2_000_000)], alice);
    expect(sbtcBalance(alice)).toBe(before);
    expect(market().vault).toBe(0);
  });

  it("merge rejects shares you do not hold", () => {
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(100)], alice);
    const r = simnet.callPublicFn(C, "merge-complete-set", [Cl.buffer(ID), Cl.uint(101)], alice);
    expect(r.result).toBeErr(Cl.uint(107));
  });

  it("resolve-idle is rejected while the window is still open", () => {
    const r = simnet.callPublicFn(C, "resolve-idle", [Cl.buffer(ID)], bob);
  ...
  it("status can only be set once", () => {
    ...
    const again = simnet.callPublicFn(C, "resolve-idle", [Cl.buffer(ID)], alice);
    expect(again.result).toBeErr(Cl.uint(102));
  });

  it("no minting after the window closes", () => {
    simnet.mineEmptyBurnBlocks(CLOSE + 5);
    const r = simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(100)], alice);
    expect(r.result).toBeErr(Cl.uint(103));
  });

  it("redeem pays the winning side only, 1:1", () => {
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(4_000_000)], alice);
    ...
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(1_000_000)], bob);

    simnet.mineEmptyBurnBlocks(CLOSE + 5);
    simnet.callPublicFn(C, "resolve-idle", [Cl.buffer(ID)], bob);

    const aliceBefore = sbtcBalance(alice);
    const r = simnet.callPublicFn(C, "redeem", [Cl.buffer(ID)], alice);
  ...
  it("redeem is rejected before resolution", () => {
    simnet.callPublicFn(C, "mint-complete-set", [Cl.buffer(ID), Cl.uint(1000)], alice);
    const r = simnet.callPublicFn(C, "redeem", [Cl.buffer(ID)], alice);
  ...
  it("redeem cannot be claimed twice", () => {
    ...
    const again = simnet.callPublicFn(C, "redeem", [Cl.buffer(ID)], alice);
